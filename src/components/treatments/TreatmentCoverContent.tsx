...
/** Linha dourada fina -- flanqueia o LB e sublinha o nome/CTA, como na
 * referência aprovada (`jato-de-plasma-capa-otimizada.webp`). */
...
export const SOFT_SHADOW = '0 1px 2px rgba(255,248,237,0.55)';

/**
 * Conteúdo padronizado da capa: logo LB, nome, descrição breve e "Ver mais
 * detalhes" -- extraído de `TreatmentCard` para ser reaproveitado também
 * pelo catálogo de Agendamento (`SchedulingTreatmentCard`), via `footerExtra`
 * para a ação de seleção. Nenhuma duplicação de estrutura/CSS entre os dois
 * lugares -- qualquer ajuste futuro na capa (tipografia, cores, alinhamento)
 * vale para os dois ao mesmo tempo.
 *
 * No mobile, cada elemento é posicionado por porcentagem exata da altura do
 * card (LB ~9%, nome ~14%, descrição ~36%, botão ~91%, footer extra perto da
...
export function TreatmentCoverContent({ treatment, onViewDetails, footerExtra }: TreatmentCoverContentProps) {
  // Quebras manuais pedidas pela cliente para estes dois tratamentos
  // específicos -- não mexem em `treatment.name`/`shortCoverDescription`
  // (usados em outros lugares: modal, aria-label, busca), só na exibição
  // visual aqui.
  const isMicroagulhamento = treatment.id === 'microagulhamento';
  const isLimpezaPremium = treatment.id === 'limpeza-pele-premium';
  ...
  return (
    ...
    >
      {/* Iluminação creme muito suave só atrás do nome/descrição -- ajuda a
          legibilidade nas capas mais claras (Dermaplaning, Skin Glass...)
          sem esconder a arte nem virar uma caixa sólida. Só no mobile. */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-1/2 top-[37%] h-[46%] w-[94%] -translate-x-1/2 -translate-y-1/2 sm:hidden"
        style={{
          background:
            'radial-gradient(ellipse at center, rgba(255,249,240,0.72) 0%, rgba(255,249,240,0.38) 55%, transparent 100%)',
        }}
      />
      ...
      {/* Área da descrição -- altura reservada igual em todos os cards
          ...
          desktop -- calibrados nas 13 descrições reais: é a combinação em
          que TODAS quebram em exatamente 2 linhas (nem 1, nem 3), da mais
          curta ("Pele limpa, saudável e radiante.") à mais longa
          ("Tecnologia avançada para uma pele lisa e livre de pelos."). Um
          ...
          `text-balance` equilibra visualmente as duas linhas. */}
      <div
        className={`absolute left-1/2 top-[36%] flex ${isLimpezaPremium ? 'w-[min(92%,230px)]' : 'w-[min(86%,178px)]'} min-h-[38px] -translate-x-1/2 items-start justify-center sm:static sm:top-auto sm:row-start-3 sm:w-full sm:min-h-[42px] sm:translate-x-0 sm:translate-y-0`}
      >
        <p
          style={{ textShadow: SOFT_SHADOW }}
          className={`line-clamp-2 w-full text-balance text-center text-[13px] font-medium leading-[1.35] text-brown sm:text-[14px] sm:leading-[1.4] ${
            isLimpezaPremium ? 'max-w-[230px]' : 'max-w-[178px] sm:max-w-[190px]'
          }`}
        >
          {isLimpezaPremium ? (
            <>
      ...
      >
        ...
        <motion.button
          ...
          className="group/btn -mx-4 -my-3 inline-flex items-center gap-1.5 rounded-full px-4 py-3 text-sm font-semibold tracking-wide text-brown-dark transition-[color,filter] duration-300 hover:text-gold-deep hover:drop-shadow-[0_0_7px_rgba(177,138,85,0.55)]"
        >
          Ver mais detalhes
          <ArrowIcon />
        </motion.button>
