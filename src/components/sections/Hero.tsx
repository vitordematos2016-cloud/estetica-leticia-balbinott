...
import heroConsultorio from '../../assets/leh-estetic/hero-leticia-consultorio.webp';
import heroProdutos from '../../assets/leh-estetic/hero-leticia-produtos.webp';
 ...
 * repouso). A imagem principal (giro 3D, raios, salto ao clique) mora em
 * `HeroVisualShowcase` -- ver esse componente para a lógica visual completa.
 */
export function Hero({ splashFinished }: HeroProps) {
  ...
  const heroImages: [{ src: string; alt: string }, { src: string; alt: string }] = [
    {
      src: heroConsultorio,
      alt: `${professionalFirstName}, especialista em estética, no consultório da ${siteContent.brand.name}`,
    },
    {
      src: heroProdutos,
      alt: `${professionalFirstName} apresentando produtos utilizados nos tratamentos da ${siteContent.brand.name}`,
    },
  ];
  ...
  return (
    ...
    >
      ...
      <Container className="relative grid items-center gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-12">
        ...
        >
          <HeroVisualShowcase images={heroImages} active={splashFinished} />
        </Reveal>
      </Container>
    </section>
  );
